...
- :literal:`Collision Precision` in units of a physical radius fraction. (default :math:`1.0^{-6}`)
...
.. math::
   ...
   \mathbf{S}_{new} = \mathbf{S}_i + \mathbf{S}_j + \mathbf{L}_{new}
...
.. math:: 
   ...
   R_{new} = \left( m_i^3 + m_j^3 \right)^{1/3}
...
The collision process is resolved during the Bulirsh-Stoer direct integration with discrete timesteps. Therefore, a collision is 
generally not detected at the exact collision time, but rather when the two particles already overlap a small amount. 
...
file. This parameter sets the tolerance of the collision detection. It is set in units of radius fraction :math:`\frac{(R_i + R_j) - r_{ij}}{R_i + R_j}`,
where :math:`R` is the physical radius and :math:`r_{ij}` the separation between the two bodies. The precision should not be set smaller than
:math:`1.0^{-10}`. When the exact time of a collision is important, a value of around :math:`1^{-6}` is recommended.
