...
__global__ void BSTTVStepAll_kernel(double4 *xold_d, double4 *vold_d, int *Transit_d, int *N_d, double *dt_d, double4 *Msun_d, int *index_d, double *time_d, double *idt_d, double *ict_d, long long timeStep, int *NBS_d, const int UseForce, const double MinMass, const int UseTestParticles, int Nst, double *TransitTime_d, int2 *NtransitsT_d){
	int idy = threadIdx.x;
	int idx = blockIdx.x;
	...
	int itransit = 1 + idx * 8;//Transit_d[idx]; //index of the transiting object
	if(Nst > 0){
		si = index_d[itransit] / def_MaxIndex;
		itransit -= NBS_d[si];
		time_d[si] = timeStep * idt_d[si] + ict_d[si] * 365.25;
	}
...
//if(idy == 0) printf("tt %d %d %d %g %g\n", idx, tt, itransit, t /dayUnit, dt1/dayUnit);
		__syncthreads();
...
//if(idy == 0) printf("tt %d %d ff %d %d %g %g\n", idx, tt, ff, itransit, t /dayUnit, dt1/dayUnit);
		for(int ff = 0; ff < 1e6; ++ff){
			__syncthreads();
			if(itransit < N_d[0]){
	
				//compute astrocentric coordinates
				double3 xa, va;		
				xa.x = x4_s[itransit].x - x4_s[0].x;
				xa.y = x4_s[itransit].y - x4_s[0].y;
				xa.z = x4_s[itransit].z - x4_s[0].z;
				va.x = v4_s[itransit].x - v4_s[0].x;
				va.y = v4_s[itransit].y - v4_s[0].y;
				va.z = v4_s[itransit].z - v4_s[0].z;

				g = xa.x * va.x + xa.y * va.y;
				gd = va.x * va.x + va.y * va.y + xa.x * a0_s[itransit * nb].x + xa.y * a0_s[itransit * nb].y;
//if(idy == 0) printf("A id %d %d %d  x %.20g y %.20g g %.20g gd %.20g -g/gd %g %.20g %g %g time %.20g\n", (idy + si * N_d[0]), tt, ff, x4_s[idy].x, x4_s[idy].y, g, gd, -g / gd, dt1 / dayUnit, dt / dayUnit, t / dayUnit, time);
				//if(-g / (gd * dt1) < 0) dt1 = -dt1; 
				if(fabs(g / gd) < fabs(dt1) && gd > 0.0 && xa.z > 0.0){
					dt1 = -g /gd;
				}
				else{
					++itransit;
...
//if(idy == 0) printf("B id %d %d %d %d %d  x %.20g y %.20g g %.20g gd %.20g -g/gd %g %.20g %g %g time %.20g\n", idx, idy, (idy + si * N_d[0]), tt, ff, x4_s[itransit].x, x4_s[itransit].y, g, gd, -g / gd, dt1 / dayUnit, dt / dayUnit, t / dayUnit, time);
				if(fabs(g / gd) < def_TransitTol){
//if(idy == 0) printf("%d %g %g %g ********* TRANSIT %d************ %g %g %g %g\n", idx, t / dayUnit, dt1 / dayUnit, time + t / dayUnit - 2457257.93116, itransit, xa.x, xa.y, x4_s[0].x, x4_s[0].y);
					if(idy == itransit){
						double rsky = sqrt(xa.x * xa.x +  xa.y * xa.y);
...
//printf("TRANSIT id %d %d R %g rsky %g T %.20g Epoch %d\n", itransit, (idy + si * N_d[0]), R, rsky, T, Epoch);
...
//printf("dx %d %d %d %d %g %g %g %g %g %g | %g %g \n", idy, tt, ff, n, dx_s[idy][0].x, dx_s[idy][0].y, dx_s[idy][0].z, dv_s[idy][0].x, dv_s[idy][0].y, dv_s[idy][0].z, t, dt1); 
				...
				__syncthreads();
//if(idy == 0) printf("error %d %d %d %d %g %g\n", idx, tt, ff, n, error_s[0], dt1);
				if(error_s[0] < def_tol * def_tol || fabs(dt1) < def_dtmin){
					...
					if(idy < N2){
						x4_s[idy] = xt_s[idy];
						v4_s[idy] = vt_s[idy];
//printf("update %d %d %d %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g %g %g tt:%d ff:%d n:%d\n", idy, idx, index_d[idi], x4_s[idy].x, x4_s[idy].y, x4_s[idy].z, v4_s[idy].x, v4_s[idy].y, v4_s[idy].z, x4_s[idy].w, v4_s[idy].w, t / dayUnit, dt1 / dayUnit, tt, ff, n);
						if(itransit == N_d[0]){
							xold_d[idi] = x4_s[idy];  
							vold_d[idi] = v4_s[idy];
//printf("Save %d\n", idx); 
