...
__global__ void BSBMStep_kernel(curandState *random_d, double4 *x4_d, double4 *v4_d, double4 *xold_d, double4 *vold_d, double *rcrit_d, double *rcritv_d, int2 *Encpairs_d, int2 *Encpairs2_d, double *dt_d, const double FGt, double2 *Msun_d, double *U_d, const int st, int *index_d, int *BSstop_d, int *Ncoll_d, double *Coll_d, double *time_d, double4 *spin_d, double3 *love_d, int *createFlag_d, const int Nst, const int NconstT, float4 *aelimits_d, unsigned int *aecount_d, unsigned int *enccount_d, unsigned long long *aecountT_d, unsigned long long *enccountT_d, int *NWriteEnc_d, double *writeEnc_d, const int UseGR, const double MinMass, const int UseTestParticles, const int SLevels, int noColl, const int NencMax){
...
//printf("EE1 %d %d %d %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g %g %d\n", si, Encpairs_d[(si * NencMax) + ii].x, Encpairs_d[(si * NencMax) + jj + l].x, xt_s[ii].w, xt_s[jj + l].w, xt_s[ii].x, xt_s[ii].y, xt_s[ii].z, xt_s[jj + l].x, xt_s[jj + l].y, xt_s[jj + l].z, delta, rcrit*rcrit, colt, Ni);
							...
							if(WriteEncounters_c[0] > 0 && noColl == 0){
								...
								double rmax = (vt_s[ii].w > vt_s[jj + l].w) ? vt_s[ii].w : vt_s[jj + l].w;
...
//printf("dRM %d %d %d %.20g %.20g %.20g | noColl: %d\n", si, i, j, d, R, dR, noColl);
						...
						Coltime_s[0] = Coltime;
//printf("ColtimeT %d %.20g %g %g %g %d %d %d %d %d\n", si, Coltime, t / dayUnit, dt1 / dayUnit, (1.0 - Coltime) * dt1, tt, ff, n, Ncol_s[0], Ncoll_d[0]);
					}
					__syncthreads();
					if(Coltime_s[0] == 10.0){
						if(idy == 0) {
