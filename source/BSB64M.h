		...
		for(int ff = 0; ff < 1e6; ++ff){
			...
			for(int n = 1; n <= 8; ++n){ 
				...
				for(int l = 0; l < NN; l += nb){
					accEnc(x4_s[ii], x4_s[jj + l], a_s[idy], rcritv_s[ii], rcritv_s[jj + l], test, ii, jj + l);
				}
				__syncthreads();
				{
					...
					if(idy == 0) {
						for(int c = 0; c < Ncol_s[0]; ++c){
							...
							if(nc >= MaxColl -1) nc = MaxColl -1;
								collide(xt_s, vt_s, i, j, Encpairs_d[(si * NmaxM) + i].x, Encpairs_d[(si * NmaxM) + j].x, Msun, U_d + sstt, test, index_d, nc, Coll_d, time + t/0.01720209895, spin_d, rcritv_s, rcrit_d, Nst, NBS, aelimits_d, aecount_d, enccount_d, aecountT_d, enccountT_d);
							}
						}
					}
