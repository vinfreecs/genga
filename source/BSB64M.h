...
__global__ void BSBMStep64_kernel(double4 *x4_d, double4 *v4_d, double4 *xold_d, double4 *vold_d, double *rcrit_d, double *rcritv_d, int2 *Encpairs_d, int2 *Encpairs2_d, double *dt_d, double FGt, double *Msun_d, double *U_d, int st, int *index_d, int *Ncoll_d, double *Coll_d, double *time_d, double3 *spin_d, const int Nst, const int NT, float4 *aelimits_d, int *aecount_d, int *enccount_d, long long *aecountT_d, long long *enccountT_d, int writeEncounters_d, double writeEncountersRadius_d, int *NWriteEnc_d, double *writeEnc_d, int UseForce, double MinMass, int UseTestParticles, int noColl){
	int idy = threadIdx.x;
	int idx = blockIdx.x;
...
//printf("BS %d %d %d %d\n", idx, st, si, N2);
	...
	if(idy < N2){
		idi = Encpairs2_d[si * NmaxM + idy].x;
		...
		for(int ff = 0; ff < 1e6; ++ff){
			...
			for(int n = 1; n <= 8; ++n){ 
				...
				for(int l = 0; l < NN; l += nb){
					accEnc(x4_s[ii], x4_s[jj + l], a_s[idy], rcritv_s, test, ii, jj + l, NN, MinMass, UseTestParticles);
				}
				__syncthreads();
				{
							...
							for(int c = 0; c < Ncol_s[0]; ++c){
								...
								if(noColl == 0){
									if(xt_s[i].w >= 0 && xt_s[j].w >= 0){
										...
										collide(xt_s, vt_s, i, j, Encpairs_d[(si * NmaxM) + i].x, Encpairs_d[(si * NmaxM) + j].x, Msun, U_d + sstt, test, index_d, nc, Coll_d, time + t / dayUnit, spin_d, rcritv_s, rcrit_d, NN, aelimits_d, aecount_d, enccount_d, aecountT_d, enccountT_d);
									}
								}
