...
//
//MEASURED 2026-09-07, RTX 5060, 100k relaxed end state, 20000 steps:
//  1.0114x end to end (19.6204 -> 19.3998 s), the pair 304.608 -> 289.657 ms.
//Real but small, and DEFAULTED OFF because that is a poor trade against the cost:
//acc4Ckick32Ab_kernel duplicates ~120 lines of acc4C_kernel, so an upstream change
//to acc4C silently diverges from the copy.
//
//That measurement understates it for a reason that does not apply on an A100. On
//the 5060 the whole working set (x4_d + v4_d + acck_d + ab_d + rcritv_d = 12.5 MB
//at 104k particles) fits in the 32 MB L2, so the acck_d round trip the fusion
//removes is an L2 hit. At 312.8k on an A100 that working set is 37.5 MB against a
//40 MB L2, and kick32Ab_kernel measures 1.548 TB/s = 99.5% of HBM peak - there the
//round trip is real DRAM traffic. The target pair is also 27.0% of the A100 step
//against 16.9% here. Modelled A100 range 1.3 - 3%, NOT yet measured.
//
//Set to 1 to build the fusion; dev/ablation.sh does that for its "+fuse"
//configuration and checks the bit identity claim byte for byte.
//
...
__device__ inline double __shfld_xor(double x, int k) {
...
__device__ inline double __shfld_up(double x, int k) {
	int2 a = *reinterpret_cast<int2*>(&x);
	a.x = __shfl_up(a.x, k);
	a.y = __shfl_up(a.y, k);
	return *reinterpret_cast<double*>(&a);
}
__device__ inline double __shfld_down(double x, int k) {
	int2 a = *reinterpret_cast<int2*>(&x);
