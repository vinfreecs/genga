...
__host__ int Data::tuneForce(int &TX){
	...
	for(int i = 0; i < 4; ++i){
		...
		//revert fg operation
		//launch with si = -1
		force <<< (NN + tx - 1) / tx, tx >>>  (x4_d, v4_d, index_d, spin_d, love_d, Msun_d, Spinsun_d, Lovesun_d, dt_d, Kt[SIn - 1], time_d, N_h[0], Nst, P.UseForce, P.UseGR, P.UseTides, 0, 0);
		cudaEventRecord(stop, 0);
		cudaEventSynchronize(stop);
		cudaEventElapsedTime(&times, start, stop); //time in microseconds
		error = cudaGetLastError();
...
__host__ int Data::step_16(int noColl){
	...
	if(ForceFlag > 0  || P.setElements > 1){
		...
		if(P.UseForce > 0 || P.UseGR > 0 || P.UseTides > 0) force <<< (N_h[0] + FrTX - 1) / FrTX, FrTX >>> (x4_d, v4_d, index_d, spin_d, love_d, Msun_d, Spinsun_d, Lovesun_d, dt_d, Kt[SIn - 1], time_d, N_h[0], Nst, P.UseForce, P.UseGR, P.UseTides, 0, 1);
...
//printf("Nencpairs2 %d\n", Nencpairs2_h[0]);
		...
		if(si < SIn - 1){
			...
			if(ForceFlag > 0){
				...
				if(P.UseForce > 0 || P.UseGR > 0 || P.UseTides > 0) force <<< (N_h[0] + FrTX - 1) / FrTX, FrTX >>> (x4_d, v4_d, index_d, spin_d, love_d, Msun_d, Spinsun_d, Lovesun_d, dt_d, Kt[si], time_d, N_h[0], Nst, P.UseForce, P.UseGR, P.UseTides, 0, 1);
...
__host__ int Data::step_small(int noColl){
	...
	if(ForceFlag > 0 || P.setElements > 1){
		...
		if(P.UseForce > 0 || P.UseGR > 0 || P.UseTides > 0) force <<< (N_h[0] + Nsmall_h[0] + FrTX - 1) / FrTX, FrTX >>> (x4_d, v4_d, index_d, spin_d, love_d, Msun_d, Spinsun_d, Lovesun_d, dt_d, Kt[SIn - 1], time_d, N_h[0] + Nsmall_h[0], Nst, P.UseForce, P.UseGR, P.UseTides, 0, 1);
...
//assume here  E = 3 or E = 4
		...
		if(si < SIn - 1){
			...
			if(ForceFlag > 0){
				...
				if(P.UseForce > 0 || P.UseGR > 0 || P.UseTides > 0) force <<< (N_h[0] + Nsmall_h[0] + FrTX - 1) / FrTX, FrTX >>> (x4_d, v4_d, index_d, spin_d, love_d, Msun_d, Spinsun_d, Lovesun_d, dt_d, Kt[si], time_d, N_h[0] + Nsmall_h[0], Nst, P.UseForce, P.UseGR, P.UseTides, 0, 1);
...
//printf("%lld %.20g %d %d\n", timeStep, time_h[0], Nencpairs_h[0], EjectionFlag2);
	...
	if(ForceFlag > 0){
		...
		if(P.UseForce > 0 || P.UseGR > 0 || P.UseTides > 0) force <<< (NT + 127) / 128, 128 >>> (x4_d, v4_d, index_d, spin_d, love_d, Msun_d, Spinsun_d, Lovesun_d, dt_d, Kt[SIn - 1], time_d, NT, Nst, P.UseForce, P.UseGR, P.UseTides, Nstart, 1);
	...
	for(int si = 0; si < SIn; ++si){
		...
		//	cudaMemcpyAsync(Nencpairs_h, Nencpairs_d, sizeof(int), cudaMemcpyDeviceToHost, copyStream);
			if(ForceFlag > 0){
				...
				if(P.UseForce > 0 || P.UseGR > 0 || P.UseTides > 0) force <<< (NT + 127) / 128, 128  >>> (x4_d, v4_d, index_d, spin_d, love_d, Msun_d, Spinsun_d, Lovesun_d, dt_d, Kt[si], time_d, NT, Nst, P.UseForce, P.UseGR, P.UseTides, Nstart, 1);
